...
// TODO: process arrived data in the first place
// NOTE: why are we deciding processed data in the configuration??????
...
#define TORQUEMENUSIZE (TORQUEMENU_LAST + 1)
  ...
  bool DoMenu(uint16_t input)
  {
    ...
    static menustruct_t menu = {
        .inedit = false, .top = 0, .selection = 0, .menusize = MAINMENUSIZE};
