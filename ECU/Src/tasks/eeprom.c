        ...
        if (BufferPos + EEPROMConfigdata[3] <= TransferSize)
        {
          ...
          if (EEPROMConfigdata[3] < 4) // data received ok, but wasn't full block. end of data.
          {
            ...
            if (checkversion((char*)Buffer)) // received data has valid header.
            {
              ...
              returnval = InitialConfig; // initialconfig = true; // rerun adc config etc for new
                                         // data in memory.
            }
...
int startupReadEEPROM(void)
{
  ...
  vTaskDelay(
      100); // Allow some time for EEPROM chip to initialise itself before start trying to access.
...
int readEEPROMAddr(uint16_t address, uint16_t size)
{
  ...
  eepromreceivedone = false;
  if (HAL_I2C_Mem_Read_IT(&hi2c2, (uint16_t)EEPROM_ADDRESS, address, I2C_MEMADD_SIZE_16BIT,
                          (uint8_t*)&EEPROMdata.buffer[address], size) != HAL_OK)
  {
...
{
  eepromreceivedone = false;
  if (HAL_I2C_Mem_Read_IT(&hi2c2, (uint16_t)EEPROM_ADDRESS, 0, I2C_MEMADD_SIZE_16BIT,
                          (uint8_t*)EEPROMdata.buffer, sizeof(EEPROMdata) + 1) != HAL_OK)
  {
    /* Reading process Error */
    Error_Handler();
  }
                        ...
                        // loop (interrupt)
{
  ...
  if (eepromwritinginprogress)
  {
    ...
    else
    {
      ...
      // count > 0
      if (eepromwritinginprogress && errorcount <= EEPROMMAXERROR)
      { // if not done sending and not in error state, start timer to next send event.
        if (HAL_TIM_Base_Start_IT(&htim16) != HAL_OK)
        {
...
bool initEEPROM(void)
{
  ...
  EEPROMQueue = xQueueCreateStatic(EEPROMQUEUE_LENGTH, EEPROMITEMSIZE, EEPROMQueueStorageArea,
                                   &EEPROMStaticQueue);
