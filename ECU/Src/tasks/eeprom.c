		...
		// Read only needs to happen at startup.
		{
			...
			if (xQueueReceive(EEPROMQueue, &msg, 0)) {
				...
				case EEPROMCurConf:
					...
					Memory_Address = &getEEPROMBlock(0)->ConfigStart
							- EEPROMdata.buffer;
					break;
				...
				case EEPROMRunningData:
					...
					Remaining_Bytes = 32;
					Memory_Address = (void*) &EEPROMdata.runtimedata
							- (void*) EEPROMdata.buffer;
					// add data dump save here.
					break;

				case writeEEPROM0:
					Remaining_Bytes = sizeof(eepromdata);
					Memory_Address = &getEEPROMBlock(1)->BlockStart
							- EEPROMdata.buffer;
					break;
				case writeEEPROM1:
					Remaining_Bytes = sizeof(eepromdata);
					Memory_Address = &getEEPROMBlock(2)->BlockStart
							- EEPROMdata.buffer;
					break;
				case writeEEPROMC:
					Remaining_Bytes = sizeof(eepromdata);
					Memory_Address = &getEEPROMBlock(0)->BlockStart
							- EEPROMdata.buffer;
					break;
...
void DoEEPROMTimeouts(void) {
	if (ReceiveInProgress && gettimer() > EEPROMConfigDataTime + MS1000) { // don't get stuck in receiving data for more than 1 second if data flow stopped.
		ReceiveInProgress = false;
		...
		if (SendInProgress && EEPROMConfigdata[0] == 30) 	// send processing.
			...
			if (EEPROMConfigdata[1] == 1) // ack
				...
				if (BufferPos < TransferSize) {
					...
					uint8_t CANTxData[8] = { 9, BufferPos >> 8, BufferPos,
							SendSize, 0, 0, 0, 0 };
					...
					uint8_t CANTxData[8] = { 9, BufferPos >> 8, BufferPos, 0, 0,
							0, 0, 0 };
			...
			} else if (EEPROMConfigdata[1] == 99) { // error.
				SendInProgress = false;
				resetSend();
			}

		} else if (ReceiveInProgress) { // in middle of receiving a data block, ignore anything else.
			...
			} else // position good, continue.
			{
				...
				if (BufferPos + EEPROMConfigdata[3] <= TransferSize) {
					...
					memcpy(&Buffer[BufferPos], (uint8_t*) &EEPROMConfigdata[4],
							EEPROMConfigdata[3]);
...
int EEPROMSend(void) {
	...
	if (TransferSize > 0) {
		uint8_t CANTxData[8] = { 8, TransferSize >> 8, TransferSize,
				EEPROMConfigdata[1], 0, 0, 0, 0 };
	...
	if (eepromwritinginprogress) {
		if (Remaining_Bytes == 0) {
			...
			if (hi2c2.State == HAL_I2C_STATE_READY && Remaining_Bytes > 0) { // i2c not busy
				...
				if (HAL_I2C_Mem_Write_IT(&hi2c2, (uint16_t) EEPROM_ADDRESS,
						Memory_Address, I2C_MEMADD_SIZE_16BIT,
						(uint8_t*) (EEPROMdata.buffer + Memory_Address),
						EEPROM_PAGESIZE) != HAL_OK) {
...
bool initEEPROM(void) {
	...
	EEPROMQueue = xQueueCreateStatic(EEPROMQUEUE_LENGTH, EEPROMITEMSIZE,
			EEPROMQueueStorageArea, &EEPROMStaticQueue);

	vQueueAddToRegistry(EEPROMQueue, "EEPROMQueue");

	timerHndlRunningData = xTimerCreate("runningdata", /* name */
	pdMS_TO_TICKS(200), /* period/time */
	pdFALSE, /* auto reload */
	(void*) 0, /* timer ID */
	saveRunningData); /* callback */

	EEPROMTaskHandle = xTaskCreateStatic(EEPROMTask,
	EEPROMTASKNAME,
	EEPROMSTACK_SIZE, (void*) 1,
