...
void CANTxTask(void* argument)
{
  ...
  FDCAN_TxHeaderTypeDef TxHeader = {.Identifier = 0, // decide on an ECU ID/
                                    .IdType = FDCAN_STANDARD_ID,
                                    .TxFrameType = FDCAN_DATA_FRAME,
                                    .DataLength =
                                        0, // only two bytes defined in send protocol, check this
  ...
  while (1)
  {
    ...
    if (xQueueReceive(CANTxQueue, &msg, waittick))
    {
      ...
      if (!CANStatus.ErrorPassive) // no point in trying to send if error passive.
      {
        ...
        //////////////////////////////////////////////////////TODO figure out what the fuck is
        /// happening
        ...
        if (HAL_FDCAN_GetTxFifoFreeLevel(hfdcanp) == 0)
        {
          ...
          if (msg.bus == bus1)
          {
            ...
            gotsem = xSemaphoreTake(bus1TXDone,
                                    5); // a bit of timeout so can't get permanently stuck here.
          }
          else
          {
...
void CANRxTask(void* argument)
{
  ...
  while (1)
  {
    ...
    if (curtick >= cycletick + CYCLETIME)
    {
      // once per cycle CAN stuff. Mostly handle timeouts, check if things received and kick
      // watchdog.
      cycletick = curtick;
      waittick = CYCLETIME;
      setWatchdogBit(watchdogBit);
...
void FDCAN1_start(void)
{
  ...
  hfdcan2p = &hfdcan2;
  if (HAL_FDCAN_Init(hfdcan2p) !=
      HAL_OK) // if can2 not initialised before filters set they are lost
  {
    // Initialization Error
    Error_Handler();
  }
...
void FDCAN2_start(void)
{
  ...
  HAL_FDCAN_ConfigGlobalFilter(hfdcan2p, FDCAN_IT_TIMEOUT_OCCURRED | FDCAN_ACCEPT_IN_RX_FIFO1,
  ...
  if (HAL_FDCAN_ActivateNotification(
          hfdcan2p, FDCAN_FLAG_ERROR_PASSIVE | FDCAN_IT_TX_COMPLETE | FDCAN_IT_RX_FIFO1_NEW_MESSAGE,
          0) != HAL_OK)
  {
    // Notification Error
    Error_Handler();
...
#if 1
  static char str[60];
  snprintf(str, 60, "InvSDOsend Id 0x%3X on %s [%2X %2X %2X %2X data %lu] (%lu)", COBSDOS_ID + id,
           bus == bus0 ? "bus0" : "bus1", msg[0], msg[1], msg[2], msg[3], data, gettimer());
  // DebugMsg(str);
#endif
  if (bus == bus0)
  {
...
char CAN_SendTimeBase(void) // sends how long since power up and primary inverter status.
{
  ...
  uint8_t CANTxData[8] = {
      ...
      Errors.CANSendError1, Errors.CANSendError2,
  ...
  uint8_t CANTxData[8] = {
      ...
      (ShutdownCircuitState() << 7) +
          HAL_FDCAN_GetTxFifoFreeLevel(
              &hfdcan2)}; // HAL_FDCAN_GetxFifoFillLevel(&hfdcan1, FDCAN_RX_FIFO0)
...
char CAN_Send4vals(uint16_t id, uint16_t val1, uint16_t val2, uint16_t val3, uint16_t val4)
{
  ...
  */
  uint8_t CANTxData[8] = {getByte(val1, 0), getByte(val1, 1), getByte(val2, 0), getByte(val2, 1),
                          getByte(val3, 0), getByte(val3, 1), getByte(val4, 0), getByte(val4, 1)};
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef* hfdcan, uint32_t RxFifo0ITs)
{
  ...
  if ((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET)
  {
    ...
    if (bufferlevel >
        25) // buffer shouldn't fill up under normal use, not sending >30 messages per cycle.
    {
...
void HAL_FDCAN_RxFifo1Callback(FDCAN_HandleTypeDef* hfdcan, uint32_t RxFifo1ITs)
{
  ...
  if ((RxFifo1ITs & FDCAN_IT_RX_FIFO1_NEW_MESSAGE) !=
      RESET) // if there is a message waiting process it
  {
...
int CheckCanError(void)
{
  ...
  if (CAN1Status.BusOff ||
      CAN1Status.ErrorPassive) // detect passive error instead and try to stay off bus till clears?
  {
    ...
    if (offcan1 == 0)
    {
      ...
      //					  offcan1count++; // increment occurances of coming off bus. if reach threshhold,
      // go to error state.
    }
  ...
  {
    if (HAL_FDCAN_Start(&hfdcan1) != HAL_OK) // add a 5 cycle counter before try again? check in
                                             // can1 send to disable whilst bus not active.
    {
      // Start Error
  ...
  if (CAN2Status.BusOff ||
      CAN2Status.ErrorPassive) // detect passive error instead and try to stay off bus till clears?
  {
    //	Errors.ErrorPlace = 0xAA;
  ...
  {
    if (HAL_FDCAN_Start(&hfdcan2) != HAL_OK) // add a 5 cycle counter before try again? check in
                                             // can1 send to disable whilst bus not active.
    {
      // Start Error
...
int initCAN(void)
{
  ...
  CANTxQueue = xQueueCreateStatic(CANTxQUEUE_LENGTH, CANTxITEMSIZE, CANTxQueueStorageArea,
                                  &CANTxStaticQueue);

  vQueueAddToRegistry(CANTxQueue, "CANTxQueue");

  CANRxQueue = xQueueCreateStatic(CANRxQUEUE_LENGTH, CANRxITEMSIZE, CANRxQueueStorageArea,
                                  &CANRxStaticQueue);

  vQueueAddToRegistry(CANRxQueue, "CANRxQueue");

  CANTxTaskHandle = xTaskCreateStatic(CANTxTask, CANTXTASKNAME, CANTXSTACK_SIZE, (void*)1,
                                      CANTXTASKPRIORITY, xCANTXStack, &xCANTXTaskBuffer);

  CANRxTaskHandle = xTaskCreateStatic(CANRxTask, CANRXTASKNAME, CANRXSTACK_SIZE, (void*)1,
