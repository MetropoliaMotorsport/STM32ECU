...
{
  xEventGroupSync(xStartupSync, 0, 1,
                  portMAX_DELAY); // ensure that tasks don't start before all initialisation done.
  ...
  while (1)
  {
    ...
    for (int i = 0; i < MAX_MOTORCOUNT; i++)
    {
      if (!InverterState[i].appc_on)
      {
        ...
        if (InverterState[i].InvState == OPERATIONAL && CarState.PRE_Done && CarState.InvRunning)
        {
          ...
          if (InverterState[i].COBID == 0xE)
            torque =
                CarState.pedalreq * TORQUESCALING; //* (CarState.MaxTorque / MAXInverterTorque);
          else
      ...
      if ((gettimer() - InverterState[i].rdo_time > 2000) && InverterState[i].appc_on &&
...
DeviceStatus InternalInverterState(
    uint16_t Status) // status 104, failed to turn on HV 200, failure of encoders/temp
{
  ...
  else if (((Status & 0b01101111) == 0b00000111) || ((Status & 0b00011111) == 0b00010011))
  { // Quick Stop Active
    return QUICKSTOP;
  }
  else if (((Status & 0b01001111) == 0b00001111) || ((Status & 0b01001111) == 0b00001001))
  { // fault reaction active, will move to fault status next
    return INERRORSTOPPING;
  }
  else if (((Status & 0b01001111) == 0b00001000) || ((Status & 0b00001000) == 0b00001000))
  { // fault status
    return INERROR;
    // send reset
  }
...
int8_t getInverterControlWord(
    const InverterState_t* Inverter) // returns response to send inverter based on current state.
{
  uint16_t TXState;
  ...
  case OFFLINE: // state 0: Not ready to switch on, no can message. Internal state only at startup.
    TXState =
        0b10000000; // send bit 128 reset message to enter state 1 in case in fault. - fault reset.
    break;
  ...
  case STOPPED: // State 2: Ready to switch on
    ...
    { // TS enable button pressed and both inverters are marked HV ready proceed to state 3.
      ...
      TXState = 0b00001111; // Lenze doesn't want to go to pre operational from stopped, have to
                            // skip straight to operational.
    }
    else
    {
  ...
  default: // unknown identifier encountered, ignore. Shouldn't be possible to get here due to
           // filters.
...
int initInv(void)
{
  resetInv(); // sets up InverterState, id's, etc, so that CAN functions will not be called till
              // setup.
  ...
  InvQueue = xQueueCreateStatic(InvQUEUE_LENGTH, InvITEMSIZE, InvQueueStorageArea, &InvStaticQueue);

  vQueueAddToRegistry(InvQueue, "InverterQueue");

  InvCfgQueue = xQueueCreateStatic(InvCfgQUEUE_LENGTH, InvCfgITEMSIZE, InvCfgQueueStorageArea,
                                   &InvCfgStaticQueue);

  vQueueAddToRegistry(InvQueue, "InverterCfgQueue");

  // if (getEEPROMBlock(0)->InvEnabled)
  InvTaskHandle = xTaskCreateStatic(InvTask, INVTASKNAME, INVSTACK_SIZE, (void*)1, INVTASKPRIORITY,
                                    xINVStack, &xINVTaskBuffer);
