...
void doVectoring(float Torque_Req, vectoradjust* adj, speedadjust* spd, int16_t pedalreq)
{
  ...
  TractionControl_U.VehicleSpeed = TorqueVectoring_U.VehicleSpeed = Regeneration_U.speed =
      IMUReceived.VelBodyX * 0.01;
  int16_t VELUSED = TractionControl_U.VehicleSpeed;

  TractionControl_U.BrakePressure =
      BrakeRear.data > BrakeFront.data ? BrakeRear.data : BrakeFront.data;
  TractionControl_U.WheelRotVelocityFL =
      getInvState(invFL)->Speed * 0.10472; // convert wheel rpm to rad/s
...
int PedalTorqueRequest(int16_t* used_pedal_percent) // returns current Nm request amount.
{
  ...
  // T 11.8.8:  If an implausibility occurs between the values of the APPSs and persists for more
  // than 100 ms
...
bool SetupInterpolationTables(eepromdata* data)
{
  ...
  if (checkversion(data->VersionString))
  {
    ...
    if (data->pedalcurves[i].PedalCurveInput[1] != 0)
    {
      ...
      for (; data->pedalcurves[i].PedalCurveInput[1] != 0;
           i++) // first number could be 0, but second will be non zero.
      {
        TorqueCurveCount++;
