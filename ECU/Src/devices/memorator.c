...
bool processTimeData(const uint8_t CANRxData[8], const uint32_t DataLength,
		const CANData *datahandle);

CANData Memorator = { &DeviceState.Memorator, MEMORATOR_ID, 3, processTimeData,
NULL, 1000 }; // [BOTS, inertia switch, BSPD.], Telemetry, front power
...
bool processTimeData(const uint8_t CANRxData[8], const uint32_t DataLength,
	...
	{
		if (DataLength == FDCAN_DLC_BYTES_6 && CANRxData[0] > 19
				&& CANRxData[1] <= 12 && CANRxData[2] <= 31
				&& CANRxData[3] <= 24 && CANRxData[4] <= 60
				&& CANRxData[5] <= 60) {
