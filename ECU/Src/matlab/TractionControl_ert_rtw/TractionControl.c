...
void TractionControl_step(void) {
	...
	if (TractionControl_U.VehicleSpeed == 0.0) {
		TractionControl_rtb_CurrentSlip_idx_0 =
				(TractionControl_rtb_CurrentSlip_idx_0
						- TractionControl_U.VehicleSpeed)
	...
	} else if (TractionControl_u0
			< TractionControl_P.Limittopositive_LowerSat) {
	 ...
	 *  Sum: '<S1>/Sum'
	 */
	if ((TractionControl_U.TC_enabled == 1.0)
			&& (TractionControl_U.VehicleSpeed > 3.0)
			&& (TractionControl_U.BrakePressure < 0.1)) {
	...
	if (rtb_DeadZone_j > TractionControl_P.TCcontroller_UpperSaturationLimit) {
		...
		if (rtb_DeadZone_j
				< TractionControl_P.TCcontroller_LowerSaturationLimit) {
			TractionControl_y =
					TractionControl_P.TCcontroller_LowerSaturationLimit;
		} else {
