...
static real_T look1_binlx(real_T TorqueVectoring_u0,
		const real_T TorqueVectoring_bp0[],
		const real_T TorqueVectoring_table[], uint32_T TorqueVectoring_maxIndex);
...
static real_T look1_binlx(real_T TorqueVectoring_u0,
	...
	if (TorqueVectoring_u0 <= TorqueVectoring_bp0[0U]) {
		...
		while (TorqueVectoring_iRght - TorqueVectoring_iLeft > 1U) {
			if (TorqueVectoring_u0
					< TorqueVectoring_bp0[TorqueVectoring_bpIdx]) {
				TorqueVectoring_iRght = TorqueVectoring_bpIdx;
			} else {
				TorqueVectoring_iLeft = TorqueVectoring_bpIdx;
			}

			TorqueVectoring_bpIdx = (TorqueVectoring_iRght
					+ TorqueVectoring_iLeft) >> 1U;
		}
...
void TorqueVectoring_step(void) {
	 ...
	 */
	if (TorqueVectoring_U.StrAngleDeg
	...
	rtb_AND_d = ((rtb_Desiredyawratereferencedegs != 0.0)
			...
			&& (TorqueVectoring_U.TorqueVectoringEnabled != 0.0)
			&& (TorqueVectoring_U.TorquePedal
	...
	rtb_Desiredyawratereferencedegs = 1.0 / TorqueVectoring_P.Sgr
			* TorqueVectoring_U.StrAngleDeg * rtb_IProdOut
			/ (rtb_IProdOut * rtb_IProdOut * TorqueVectoring_P.Ku
					+ TorqueVectoring_P.l);
	 ...
	 *  Product: '<S6>/Product1'
	 */
	rtb_IProdOut = 1.0 / rtb_IProdOut
			* (TorqueVectoring_P.myy * TorqueVectoring_P.g)
			* TorqueVectoring_P.Gain2_Gain;
	...
	if (!(fabs(rtb_Desiredyawratereferencedegs) < rtb_IProdOut)) {
		if (rtIsNaN(rtb_Desiredyawratereferencedegs)) {
			...
			rtb_Desiredyawratereferencedegs = (rtb_Desiredyawratereferencedegs
					> 0.0);
		}
	 ...
	 *  MATLAB Function: '<S6>/MATLAB Function'
	 */
	rtb_IProdOut = rtb_Desiredyawratereferencedegs
			- TorqueVectoring_P.Gain_Gain * TorqueVectoring_U.VehicleYawRate;
	...
	} else if (rtb_Desiredyawratereferencedegs
			< TorqueVectoring_P.LowerYawMoment) {
	 ...
	 *  Inport: '<Root>/StrAngleDeg'
	 */
	rtb_Igains = TorqueVectoring_U.StrAngleDeg
			* TorqueVectoring_P.feedforwardgain;
	...
	rtb_Yawmomentsaturation = (real_T) (TorqueVectoring_U.FeedbackEnabled
			&& rtb_AND_d) * rtb_Yawmomentsaturation
			+ (real_T) (TorqueVectoring_U.FeedForwardEnabled && rtb_AND_d)
					* rtb_Igains;
	...
	rtb_Igains = TorqueVectoring_P.Gain3_Gain * rtb_Yawmomentsaturation
			* TorqueVectoring_P.Rd
			/ (2.0 * TorqueVectoring_P.wr * TorqueVectoring_P.Gr)
	...
	rtb_Igains = TorqueVectoring_P.Gain2_Gain_o * rtb_Yawmomentsaturation
			* TorqueVectoring_P.Rd
			/ (2.0 * TorqueVectoring_P.wf * TorqueVectoring_P.Gr)
	 ...
	 */
	rtb_IProdOut *= 1.0 / TorqueVectoring_P.TuneGains
			* look1_binlx(TorqueVectoring_U.VehicleSpeed,
					TorqueVectoring_P.Igains_bp01Data,
	 ...
	 */
	if (rtb_Desiredyawratereferencedegs
			> TorqueVectoring_P.Clamping_zero_Value) {
	 ...
	 *  Switch: '<S36>/Switch2'
	 */
	if ((TorqueVectoring_P.Clamping_zero_Value
	 ...
	 */
	TorqueVectoring_DW_l.Integrator_DSTATE +=
			TorqueVectoring_P.Integrator_gainval * rtb_IProdOut;
}
